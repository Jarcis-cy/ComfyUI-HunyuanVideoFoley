...
class AttributeDict:
    def __init__(self, data: Union[Dict, List, Any]):
        if isinstance(data, dict):
            ...
            self._list = [AttributeDict(item) if isinstance(item, (dict, list)) else item for item in data]
        else:
            self._value = data
    ...
    def to_dict(self) -> Union[Dict, List, Any]:
        if hasattr(self, "_list"):
            return [item.to_dict() if isinstance(item, AttributeDict) else item for item in self._list]
        elif hasattr(self, "_value"):
            return self._value
        else:
            result = {}
